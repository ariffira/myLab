        ...
        <div class="form-group">
          ...
          <div class="col-sm-9">
            <select id="searchField" name="search_field" class="form-control">
              <?php if (count($admins) > 0) : foreach ($admins[0] as $field => $value) : ?>
      ...
      <table class="table table-condensed table-hover table-striped">
        ...
        <tbody>
          <?php foreach ($admins as $row) : ?>
            <tr onclick="javascript:window.location='<?php echo site_url('admin/admin/entry/'.$row->id); ?>';">
    ...
    <div class="row box-body">
      <div class="col-md-12">
        ...
        <?php foreach ($admins as $row) : ?>
                ...
                <div class="row">
                  ...
                  <div class="col-sm-8" style="word-wrap: break-word;">
                    <?php if (in_array($field, Mod::$activation_fields)) : ?>
                      <?php echo activation_field($value, $field, 'admin/update_field/'.$row->id); ?>
                    <?php elseif (in_array($field, Mod::$country_fields)) : ?>
                      <?php echo country_field($value, $field, 'admin/update_field/'.$row->id); ?>
                    <?php elseif (in_array($field, Mod::$speciality_fields)) : ?>
                      <?php echo speciality_field($value, $field, 'admin/update_field/'.$row->id); ?>
                    <?php elseif (in_array($field, Mod::$access_permission_fields)) : ?>
                      <?php echo access_permission_field($value, $field, 'admin/update_field/'.$row->id); ?>
                    <?php elseif (in_array($field, Mod::$patient_package_fields)) : ?>
                      <?php echo patient_package_field($value, $field, 'admin/update_field/'.$row->id); ?>
                    <?php else : ?>
                      <?php echo !is_array($value) ? text_field($value, $field, 'admin/update_field/'.$row->id) : 'Array'; ?>
                    <?php endif; ?>
                  </div>
                </div>
              </div>
          ...
          <div class="row">
            <?php foreach (Mod::$admin_online_modules as $module) : $module = (object)$module; ?>
              <div class="col-md-6">
                <div class="row">
                  ...
                  <div class="col-sm-5" style="word-wrap: break-word;">
                    <?php echo activation_field(isset($row->modules[$module->module]) && $row->modules[$module->module]->activate ? TRUE : FALSE, $module->module, 'admin/activate_module/'.$row->id); ?>
                  </div>
                </div>
              </div>
            <?php endforeach; ?>
