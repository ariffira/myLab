...
class Mopack extends CI_Model {
  ...
  function __construct()
  {
    ...
    $this->_count_all = $this->mod->port->p->count_all('packages');
  }
  ...
  public function get_id($id)
  {
    ...
    $query = $this->mod->port->p->get_where('packages', array('id' => $id, 'for' => $this->mod->user_role() == 'doctor' ? 2 : 1), 1);

    if ($query->num_rows() > 0)
    {
      $row = $query->row();

      $fields = $this->mod->port->p->field_data('packages');
      ...
      $query = $this->mod->port->p->get_where('packages_terms', array('for' => $row->id, ));
      if ($query->num_rows() > 0)
      {
  ...
  public function get_name($name)
  {
    ...
    switch ($this->mod->user_role()) {
      case 'doctor':
        $query = $this->mod->port->p->get_where('packages', array('name' => $name, 'for' => 2, ), 1);
        break;

      case 'patient':
        $query = $this->mod->port->p->get_where('packages', array('name' => $name, 'for' => 1, ), 1);
        break;
      
      default:
        $query = $this->mod->port->p->get_where('packages', array('name' => $name, ), 1);
        break;
    }
  ...
  public function get_list()
  {
    ...
    $this->mod->port->p->db_select();
    $this->mod->port->p->order_by('visual_order', 'ASC');

    $query = $this->mod->port->p->get('packages');
    ...
    if ($query->num_rows() > 0)
    {
      ...
      foreach ($result as $index => $row)
      {
        ...
        if ($this->mod->user_role() == 'doctor' && $row->for == 2 || $this->mod->user_role() == 'patient' && $row->for == 1)
        {
  ...
  public function update_package()
  {
    ...
    if ($insert || $update)
    {
      ...
      $this->mod->port->p->set('for'                 , $for );
      $this->mod->port->p->set('name'                , $name );
      $this->mod->port->p->set('display_name'        , $display_name );
      $this->mod->port->p->set('intro'               , $intro );
      $this->mod->port->p->set('intro_top_right'     , $intro_top_right );
      $this->mod->port->p->set('price_cent'          , $price_cent );
      $this->mod->port->p->set('running_time_type'   , $running_time_type );
      $this->mod->port->p->set('running_time'        , $running_time );
      $this->mod->port->p->set('running_time_quant'  , $running_time_quant );
      $this->mod->port->p->set('cancel_buffer_type'  , $cancel_buffer_type );
      $this->mod->port->p->set('cancel_buffer'       , $cancel_buffer );
      $this->mod->port->p->set('cancel_buffer_quant' , $cancel_buffer_quant );
      $this->mod->port->p->set('visual_order'        , $visual_order );
      $this->mod->port->p->set('visual_class'        , $visual_class );
      $this->mod->port->p->set('visual_new_row'      , $visual_new_row ? 1 : 0 );
      ...
      if ($no_buffers && is_array($no_buffers))
      {
        ...
        $this->mod->port->p->set('no_buffers' , implode(',', $no_buffers));
      }
      else
      {
        $this->mod->port->p->set('no_buffers' , '');
      }
      ...
      if ($activating_modules && is_array($activating_modules))
      {
        ...
        $this->mod->port->p->set('activating_modules' , implode(',', $activating_modules));
      }
      else
      {
        $this->mod->port->p->set('activating_modules' , '');
      }
      ...
      if ($id)
      {
        ...
        $db_result = $this->mod->port->p->update('packages');
        ...
        if ($term_id && is_array($term_id) && count($term_id) > 0)
        {
          foreach ($term_id as $tid => $trow)
          {
            ...
            if ($tid)
            {
              ...
              {
                $this->mod->port->p->where('id', $tid);
                $this->mod->port->p->limit(1);
                $db_result = $this->mod->port->p->delete('packages_terms');
              }
              else
              {
                $this->mod->port->p->set('for'   , $term_for[$tid]);
                $this->mod->port->p->set('name'  , $term_name[$tid]);
                $this->mod->port->p->set('intro' , $term_intro[$tid]);

                $this->mod->port->p->where('id', $tid );
                $this->mod->port->p->limit(1);

                $this->mod->port->p->update('packages_terms');
              }
            }
            else
            {
              if ($term_insert && is_array($term_insert) && count($term_insert) > 0 && isset($term_insert[$tid]) && $term_insert[$tid])
              {
                ...
                $this->mod->port->p->insert('packages_terms');
              }
            }
          }
        }
      ...
      {
        $db_result = $this->mod->port->p->insert('packages');
      }
    }
    if ($delete)
    {
      if ($id)
      {
        $this->mod->port->p->where('for', $id );
        $this->mod->port->p->delete('packages_terms');

        $this->mod->port->p->where('id', $id );
        $this->mod->port->p->limit(1);
        $db_result = $this->mod->port->p->delete('packages');
      }
    }
  ...
  public function change_package()
  {
    ...
    if (!$this->mod->user_role() || !$this->mod->user())
    {
      return (($msg->status = 'ERROR') && ($msg->msg = 'You are not logged in.')) ? $msg : $msg;
    }

    Minvoice::$role = $this->mod->user_role() == 'doctor' ? Minvoice::DOCTOR_INVOICE : Minvoice::PATIENT_INVOICE;
    $invoices = $this->minvoice->get_invoice($this->mod->user());
    ...
    $curr_name = $this->mod->user()->package;
    ...
    {
      $possible_next_time = $this->possible_next_time($this->mod->user()->package, count($invoices) > 0 && count($invoices[0]) > 0 ? strtotime($invoices[0][0]->cdate) : time());
    }
    ...
    $this->mod->port->p->set('package'                     , $package->name);
    $this->mod->port->p->set('ref'                         , random_string('alnum', 20));
    $this->mod->port->p->set('payment_title'               , $this->encrypt->encode($title));
    $this->mod->port->p->set('payment_gender'              , $this->encrypt->encode($gender));
    $this->mod->port->p->set('payment_first_name'          , $this->encrypt->encode($first_name));
    $this->mod->port->p->set('payment_last_name'           , $this->encrypt->encode($last_name));
    $this->mod->port->p->set('payment_street'              , $this->encrypt->encode($street));
    $this->mod->port->p->set('payment_street_additional'   , $this->encrypt->encode($street_additional));
    $this->mod->port->p->set('payment_postal_code'         , $this->encrypt->encode($postal_code));
    $this->mod->port->p->set('payment_locality'            , $this->encrypt->encode($locality));
    $this->mod->port->p->set('payment_email'               , $this->encrypt->encode($email));
    $this->mod->port->p->set('payment_telephone'           , $this->encrypt->encode($telephone));
    $this->mod->port->p->set('cdate'                       , date('Y-m-d H:i:s', $possible_next_time));

    if ($this->mod->user_role() == 'doctor')
    {
      $this->mod->port->p->set('doctor_id', $this->mod->user_id());
      $this->mod->port->p->insert('doctors_payment');

      $query = $this->mod->port->p->get_where('doctors_payment', array('id' => $this->mod->port->p->insert_id(), ), 1);
    }

    if ($this->mod->user_role() == 'patient')
    {
      $this->mod->port->p->set('patient_id', $this->mod->user_id());
      $this->mod->port->p->insert('patients_payment');

      $query = $this->mod->port->p->get_where('patients_payment', array('id' => $this->mod->port->p->insert_id(), ), 1);
    }
