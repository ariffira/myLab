$(document).ready(function() {
  ...
  owlCalendarAll.each(owlCalendarInitCallback = function() {
    ...
    !doctorId ? (function() {
      ...
      var maxTerminPerDay = owlCalendar.hasClass('large') ? 8 : 4;
      ...
      for (var dayDelta = 0, thisDay = startDate.clone(); dayDelta < 28; dayDelta++, thisDay.add('d', 1)) {
        ...
        $('<div></div>').addClass('day').html(thisDay.format('dd')).appendTo(colHeader);
        $('<div></div>').addClass('date').html(thisDay.format('DD.MM.YY')).appendTo(colHeader);
  ...
  $('select[name="insurance_provider"], select[name="treatment"], .chosen-select').chosen({
              ...
              for (var i = 0, evt; evt = data[i++];) {
                for (var week = 0; week < 52; week++) {
                  if (evt.repetitive && parseInt(evt.repetitive) || week == 0) {
                    ...
                    vc ? ((poolPushEvent.className = 'btn-success'), (poolPushEvent.textColor = 'white'), (poolPushEvent.title = 'Schließzeiten')) : $.termin;
        ...
        eventDrop: function(event, delta, revertFunc) {
          ...
          if (confirm("Sind Sie sicher, zu dieser Änderung?")) {
            if (event.source.name == "termins") {
              ...
              $.ajax({
                type: "POST",
                url: $.siteUrl + '/termin/doctor/calendar/update_termin_time',
                data: postData,
                async: true,
                cache: false,
                beforeSend: function() {
