...
class Usertracking
{
  ...
  private $needed_fields = array(
    ...
    array('name' => 'user_identifier', 'type' => 'string', 'forge_type' => 'int', 'forge_constraint' => '255'),
    ...
    array('name' => 'user_role', 'type' => 'string', 'forge_type' => 'varchar','forge_constraint'=>'255','null' => TRUE,)
);
...
//   echo "fas";
  ...
  public function track_this()
  {
    ...
    $input_data['referer_page'] = $this->CI->agent->referrer();
     ...
     $user_id = $this->CI->m->user_id();
     $input_data['user_identifier']=$user_id;
    
     $user_role=$this->CI->m->user_role();
...
//    }
    ...
    $this->CI->load->database();
    $a= base_url('index.php/akte/notification');
    $b=base_url('index.php/akte/autologin');
    $match=array($a,$b);
    foreach ($match as $k=>$v){
        if(strpos($v,$input_data['request_uri'])!==FALSE){
            $status=TRUE;
            break;
        }
        else{
