...
class Mdoc extends CI_Model {
  ...
  */
     ...
     public function get_profile_img_path($regid = NULL)
  {
   ...
   public function get_doc_associate_image($regid = NULL,$value=null)
  {
    if (!empty($regid) && $value!="")
    {
      $this->m->port->m->db_select();
      $this->m->port->m->limit(1);
      $doc = $this->get($value, 'id');
      if (!empty($doc))
      {
        $doc = is_array($doc) ? $doc[0] : ( is_object($doc) && !empty($doc->document_name) && !empty($doc->document_extension) ? $doc : FALSE );
        ...
        if ($doc)
        {
          $location = './protected/uploads/doctor/'.bin2hex($this->aes_encrypt->en($regid)).'/'.$doc->document_name.'.'.$doc->document_extension;
