...
class Moterm extends CI_Model {
  ...
  function __construct()
  {
    ...
    if (!$this->m->port->p->table_exists('terms'))
    {
      $alert .= '<p>';
    ...
    $this->_count_all = $this->m->port->p->count_all('terms');
  }
  ...
  public function get_id($id)
  {
    ...
    $query = $this->m->port->p->get_where('terms', array('id' => $id, ), 1);

    if ($query->num_rows() > 0)
    {
      $row = $query->row();

      $fields = $this->m->port->p->field_data('terms');
  ...
  public function get_name($name)
  {
    ...
    $query = $this->m->port->p->get_where('terms', array('name' => $name, ), 1);
  ...
  public function update_term()
  {
    ...
    if ($insert || $update)
    {
      ...
      if ($id)
      {
        ...
        $db_result = $this->m->port->p->update('terms');
      }
      else
      {
        $db_result = $this->m->port->p->insert('terms');
      }
    }
    if ($delete)
    {
      ...
      {
        $this->m->port->p->where('id', $id );
        $db_result = $this->m->port->p->delete('terms');
      }
    }
          ...
          <div class="modal-dialog modal-lg">
            <div class="modal-content">
              ...
              <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary" '.($in ? ($print ? 'data-toggle="print-ready"' : 'data-toggle="print"') : 'data-toggle="print-redirect" value="'.$term->name.'" data-target="'.$term->name.'/'.$term->id.'"').'>
