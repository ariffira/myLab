  ...
  public function page($post = FALSE)
  {
    ...
    if ($post)
    {
      if ($this->form_validation->run('login'))
      {
         ...
         $redirect=$this->m->force_pass_change($email,$pass, true);
         if($redirect!='' && !empty($redirect) && $redirect){
                redirect($redirect);
         }

        if ($this->m->login($email, $pass))
        {
          ...
          $redirect=$this->m->login_changepassword($email,$pass, true);
          $this->m->updateIp($email,true);
          redirect($redirect);
          return;
        }
        else
