<?php
class Gapi_model extends CI_Model {

    private $auth_token_key = 'gapi_auth_token';
    private $client = null;
	...
	$this->dbi = $this->load->database('personal', TRUE);
	...
	try {
	    ...
	    $this->client->setScopes($this->scopes[0]);
	} catch (Exception $e) {
	    $this->unsetAuthToken();
	}
    }

    public function isGAuth() {
	...
	if (!$this->getAuthToken()) {
	    ...
	    /* $auth_token=  $this->getAuthToken();
	      $this->client->refreshToken($auth_token);
	      $this->setAuthToken($this->client->getAccessToken());
	     */
	}
	return $output;
    ...
    //
    private function deleteEvent($id, $showerror = 0) {
	$this->client->setAccessToken($this->getAuthToken());
	$service = new Google_Service_Calendar($this->client);
	try {
    ...
    // who is context of syncing ie. "docter" or "patient" ,"ID is patient/docter id"
