...
class Ddanalysis extends MX_Controller {
...
//    $_ci->m->port->m->where('document_date <=', date('Y-m-d H:i:s', time()));
  ...
  public function drug_interaction()

  {
    ...
    $this->m->role_diff(
      function() use ($_ci,$subs,$nam){
        if (!$_ci->m->us_id())
        {
          $_ci->load->view('not_chosen_view', array(), TRUE);
          return;
        }

        $_ci->load->model('ddanalysis/mddanalysis');
        $_ci->load->model('medication/mmedication');

        $drug_id = $_ci->mddanalysis->get_drug($subs,$nam);
        //var_dump($drug_id);
        //foreach ($drugs as $drug) {
          //$drug_id= $drug->id;
        //}

        $medication_list = $_ci->mmedication->get_current_medication();

        //$drug_interactions = $_ci->mddanalysis->get_drug_interaction($drug_id);
        $infos = $_ci->mddanalysis->get_all_info($drug_id);
        //$food_interactions = $_ci->mddanalysis->get_food_interaction($drug_id);
        //print_r($infos);

        $_ci->load->view('ddanalysis/ddanalysis_view', array(
          'given_substance' => $subs,
          'given_name' => $nam,
          'infos' => $infos,
          'drugs' => $drugs,
...

  {
    ...
    $atc_list = $_ci->mmedication->get_current_medication();
    $drug_list = $_ci->mmedication->get_current_medication();
