     ...
     <?php
         ...
         /* $this->ui->feed_item->title('content', '<a href="javascript:;">'.date('d.m.Y', strtotime($entry->document_date)).'</a>'.' - befinden: '.'<a href="javascript:;">'.$entry->title.'</a>');*/
          ...
          $this->ui->feed_item->content(
            'content',
            $this->load->view('familyhistory/familyhistory_home_view', array(
              'entry' => $entry,
              'hide_insert' => TRUE,
              'static' => TRUE, 
              'readonly' => FALSE,
...
<?php
if(empty($tot_record)&&$show_more==5)
{
    
}
if(!empty($tot_record)&&!empty($entries)&&$tot_record>5 && count($entries)>=5)
{
echo "<a href='".site_url("akte/familyhistory/feed/?id=purple&showmore=$show_more")."' class='show_more' id='show_more".$show_more."' ><div class='btn btn-default pull-right'>Show More</div></a>";
}
?>
<?php else: ?>
