...
class mdoc extends CI_Model
{
  ...
  */
   ...
   */
    public function get_upload_path()
    {
        ...
        if ($_ci->mod->user() && $_ci->mod->user()->regid) {
            return './protected/uploads/'.$this->mod->role_diff(
                function () {
                    return 'doctor';
                },
                function () {
            ...
            ).'/'.bin2hex($_ci->aes_encrypt->en($_ci->mod->user()->regid)).'/';
        }

        return false;
    }
    public function get_doctor_img_path($id = null)
    {
        ...
        if (!empty($row->profile_image) && !empty($row->regid)) {
            ...
            if (!empty($doc)) {
                ...
                if ($doc) {
                    $location = './protected/uploads/doctor/'.bin2hex($this->aes_encrypt->en($row->regid)).'/'.$doc->document_name.'.'.$doc->document_extension;
    ...
    //echo $regid;
        ...
        $query = $this->mod->port->p->get_where('patients', array('regid = ' => $regid), 1);
   ...
   */
    public function get_us_upload_path()
    {
        ...
        if ($_ci->mod->us() && $_ci->mod->us()->regid) {
            return './protected/uploads/patient/'.bin2hex($_ci->aes_encrypt->en($_ci->mod->us()->regid)).'/';
        }
   ...
   */
    ...
    {
        if ($this->mod->us() && $this->mod->us()->regid) {
            return './protected/uploads/patient/'.bin2hex($this->aes_encrypt->en($this->mod->us()->regid)).'/'.$row->document_name.'.'.$row->document_extension;
        }

        if ($this->mod->user() && $this->mod->user()->regid) {
            return './protected/uploads/patient/'.bin2hex($this->aes_encrypt->en($this->mod->user()->regid)).'/'.$row->document_name.'.'.$row->document_extension;
        }
    ...
    public function get_profile_image_path($row = null)
    {
        ...
        if (!empty($row->profile_image) && !empty($row->regid)) {
            ...
            if (!empty($doc)) {
                ...
                if ($doc) {
                    $location = './protected/uploads/'.$this->mod->role_diff(
                        function () {
                            return 'doctor';
                        },
                        function () {
   ...
   */
    public function do_upload($patient_id, $role = 'patient', $upload_field = 'document_upload')
    {
        ...
        $config['upload_path'] = './protected/uploads/'.trim($role, '/').'/'.bin2hex(
            $this->aes_encrypt->en(
                $this->mod->role_diff(
                    function () use ($_ci, $role) {
                        if ($role == 'patient') {
                            if ($_ci->mod->us()) {
                                return $_ci->mod->us()->regid;
                            }

                            return $_ci->mod->user()->regid;
                        }
        ...
        $config['allowed_types'] = 'jpg|png|jpeg|tif|doc|pdf|docx|odt';
  ...
  */
   ...
   */
    public function get($value, $field = 'patient_id')
    {
        ...
        Mod::$encrypted_fields_bin2hex = true;

        $ret = $this->mod->get('m', 'patients_docs', $condition, self::$encrypted_fields);
  ...
  */
   ...
   */
    ...
    {
        $this->mod->db_set('m', $insert_params, self::$plain_fields, self::$datetime_fields, self::$encrypted_fields);

        $this->mod->port->m->insert('patients_docs');

        return $this->mod->port->m->insert_id();
    }
  ...
  */
   ...
   */
    ...
    {
        if (!$this->mod->db_where('m', $id)) {
            return false;
        }

        $this->mod->port->m->limit(1);

        $this->mod->db_set('m', $update_params, self::$plain_fields, self::$datetime_fields, self::$encrypted_fields);

        return $this->mod->port->m->update('patients_docs');
    }
  ...
  */
   ...
   */
    public function delete($id)
    {
        ...
        return $this->mod->port->m->delete('patients_docs');
    }
}
