...
class Mopayone extends CI_Model {
  ...
  public function post($msg, $type = 'elv')
  {
    ...
    $data = array_merge($data, array(
      ...
      // ---------
      'customerid'      => $this->mod->user_value('regid'), // - AN..20 Merchant's customer ID (Permitted symbols: 0-9, a-z, A-Z, .,-,_,/)
      'userid'          => $this->mod->user_value('p1_userid'), // - N..12 Debtor ID (PAYONE)
      ...
      'email'           => $this->mod->user_value('email'), // - AN..50 Email address (Mandatory for payment type BSV, KLV)
    ...
    if ($msg->status == 'POST')
    {
      ...
      if ($result->status != 'APPROVED')
      {
        ...
        $this->mod->port->p->where('id', $payment->id);
        $this->mod->port->p->limit(1);

        if ($this->mod->user_role() == 'doctor')
        {
          $this->mod->port->p->delete('doctors_payment');
        }

        if ($this->mod->user_role() == 'patient')
        {
          $this->mod->port->p->delete('patients_payment');
        }
      ...
      else
      {
        ...
        $this->mod->port->p->set('txid'                      , $result->txid);
        $this->mod->port->p->set('userid'                    , $result->userid);
        $this->mod->port->p->set('cdate'                     , date('Y-m-d H:i:s', time()));
        ...
        $this->mod->port->p->set('p1_userid', $result->userid);
        $this->mod->port->p->set('package', $package->name);

        $this->mod->port->p->where('id', $this->mod->user_id());
        $this->mod->port->p->limit(1);
        ...
        if (isset($package->activating_modules) && is_array($package->activating_modules) && count($package->activating_modules) > 0)
        {
          ...
          Module::$role = $this->mod->user_role() == 'doctor' ? Module::DOCTOR_MODULES : Module::PATIENT_MODULES;
          foreach ($package->activating_modules as $m) {
            $this->module->activate($this->mod->user_id(), $m);
          }
        }
      }
    }
    ...
    if (is_array($result->data) && count($result->data) > 0)
    {
      foreach ($result->data as $key => $value)
      {
        ...
        $this->mod->port->p->set('payment_id', $payment->id);
        $this->mod->port->p->set('key', $this->encrypt->encode($key));
        $this->mod->port->p->set('value', $this->encrypt->encode($value));
        
        if ($this->mod->user_role() == 'doctor')
        {
          $this->mod->port->p->insert('doctors_payment_p1meta');
        }

        if ($this->mod->user_role() == 'patient')
        {
          $this->mod->port->p->insert('patients_payment_p1meta');
        }
      }
    }
  ...
  public function ts_insert()
  {
    ...
    $this->mod->port->p->set('txaction'     , $this->encrypt->encode($txaction));
    $this->mod->port->p->set('mode'         , $this->encrypt->encode($mode));
    $this->mod->port->p->set('portalid'     , $this->encrypt->encode($portalid));
    $this->mod->port->p->set('aid'          , $this->encrypt->encode($aid));
    $this->mod->port->p->set('clearingtype' , $this->encrypt->encode($clearingtype));
    $this->mod->port->p->set('txtime'       , $this->encrypt->encode($txtime));
    $this->mod->port->p->set('currency'     , $this->encrypt->encode($currency));
    $this->mod->port->p->set('userid'       , $this->encrypt->encode($userid));
    $this->mod->port->p->set('customerid'   , $this->encrypt->encode($customerid));
    $this->mod->port->p->set('param'        , $this->encrypt->encode($param));

    $this->mod->port->p->insert('p1_ts');

    $insert_id = $this->mod->port->p->insert_id();
    ...
    $query = $this->mod->port->p->get_where('p1_ts', array('id' => $insert_id, ), 1);
    if ($query->num_rows() <= 0)
    {
      return FALSE;
    }
    ...
    foreach ($this->input->post() as $key => $value)
    {
      ...
      {
        $this->mod->port->p->set('ts_id', $ts->id);
        $this->mod->port->p->set('key', $this->encrypt->encode($key));
        $this->mod->port->p->set('value', $this->encrypt->encode($value));

        $this->mod->port->p->insert('p1_ts_meta');
      }
    }
  ...
  {
    $this->mod->port->p->db_select();
    $query = $this->mod->port->p->get('p1_ts');
    ...
    if ($query->num_rows())
    {
      ...
      {
        $query = $this->mod->port->p->get_where('p1_ts_meta', array('ts_id' => $row->id, ));
