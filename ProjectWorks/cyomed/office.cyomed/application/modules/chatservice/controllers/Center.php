...
class Center extends MX_Controller {
  ...
  public function index()
  {
    ...
    $page_content = $this->load->view('center_view', $page_data, TRUE);
  ...
  */
  ...
  public function update_callback_time($id){
    $date = date('Y-m-d H:i:s');
    $this->mod->port->p->update('care_chatservice',array('care_doctor_callback_patient_init_time'=>$date),array('id'=>$id,'care_doctor_callback_patient_init_time'=>0));
  }
  ...
  */
  public function update_specialization(){
     ...
     $this->load->model('videochat');
     $doctors=array();
     $doctors = $this->videochat->get_doctor_for_specialization($specialization);
  ...
  public function img_src(){
    $regid = trim(strip_tags($_GET['regid']));
    $care_id = trim(strip_tags($_GET['care_id']));
    if (!$regid)
    {
      echo 'No Matches Found';
      return;
    ...
    $img_path = $this->mdoc->get_profile_img_path($regid);
    $img_path = base_url($img_path);
    $this->update_callback_time($care_id);
    echo json_encode(array('src'=>$img_path));
  }
  ...
  public function get_new_request(){
    ...
    $this->load->model('videochat');
    $patient = $this->videochat->get_patients_list($care_id);
  ...
  public function get_service_for_notification(){
    $this->mod->port->p->select('id');
    $query = $this->mod->port->p->get_where('quickblox',array('role'=>'role_service'));
    if($query->num_rows()>0){
