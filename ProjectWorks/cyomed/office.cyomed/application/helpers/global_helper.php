...
if (!function_exists('ajax_redirect')) {
    function ajax_redirect($redirect)
    {
        ...
        redirect('akte/akte/active_url?r='.rawurlencode($_ci->encrypt->encode($redirect)));
    }
}
...
if (!function_exists('rezept_ajax_redirect')) {
    function rezept_ajax_redirect($redirect)
    {
        ...
        redirect('rezept/rezept/active_url?r='.rawurlencode($_ci->encrypt->encode($redirect)));
    }
}
...
/*
 * output ajax request subpages, or output also structure
 *
 * @param string $page_content
 */
if (!function_exists('output_ajax')) {
    function output_ajax($page_content = '')
    {
        static $ci;
        ...
        if ($ci->input->is_ajax_request()) {
            $ci->output->set_output($page_content);
        } else {
            $page_c = $ci->load->module('auth/page');

            $ci->page->_page($page_content);
        }
    }
}
