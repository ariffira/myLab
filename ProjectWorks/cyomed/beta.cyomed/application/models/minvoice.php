...
class Minvoice extends CI_Model {
  ...
  */
  ...
  function __construct()
  {
    ...
    $this->m->port->p->set('package', 'free');
  ...
  public function get_payment($value, $field = NULL)
  {
    ...
    {
      $this->m->port->p->where($field, $value);
    }
    if (self::$role === self::PATIENT_INVOICE)
    {
    ...
    $this->m->port->p->order_by('cdate', 'DESC');
    $query = $this->m->port->p->get($table);
    ...
    if ($query->num_rows() > 0)
    {
      ...
      foreach ($result as $index => $row)
      {
        ...
        $query = $this->m->port->p->get_where($table.'_p1meta', array('payment_id' => $result[$index]->id, ));
        if ($query->num_rows() > 0)        
        {
    ...
    if ($top_pack->running_time_type == 'static' && $top_pack->running_time > 0)
    {
      while (time() > $expire_time)
      {
        ...
        if (time() > $expire_time)
        {
          if (self::$role === self::PATIENT_INVOICE)
          {
            $query = $this->m->port->p->get_where('patients', array('id' => $user->id, ), 1);
          }
          if (self::$role === self::DOCTOR_INVOICE)
          {
            $query = $this->m->port->p->get_where('doctors', array('id' => $user->id, ), 1);
          }
          $user_row = $query->row();

          $this->m->port->p->set('package'                     , $user_row->package);
          // $this->m->port->p->set('payment_account_holder'      , $user_row->payment_account_holder);
          // $this->m->port->p->set('payment_bank_name'           , $user_row->payment_bank_name);
          // $this->m->port->p->set('payment_account_number'      , $user_row->payment_account_number);
          // $this->m->port->p->set('payment_bank_code'           , $user_row->payment_bank_code);
          $this->m->port->p->set('payment_title'               , $user_row->payment_title);
          $this->m->port->p->set('payment_gender'              , $user_row->payment_gender);
          $this->m->port->p->set('payment_first_name'          , $user_row->payment_first_name);
          $this->m->port->p->set('payment_last_name'           , $user_row->payment_last_name);
          $this->m->port->p->set('payment_street'              , $user_row->payment_street);
          $this->m->port->p->set('payment_street_additional'   , $user_row->payment_street_additional);
          $this->m->port->p->set('payment_postal_code'         , $user_row->payment_postal_code);
          $this->m->port->p->set('payment_locality'            , $user_row->payment_locality);
          $this->m->port->p->set('payment_email'               , $user_row->payment_email);
          $this->m->port->p->set('payment_telephone'           , $user_row->payment_telephone);
          
          $this->m->port->p->set('cdate'                       , date('Y-m-d H:i:s', $expire_time += 1));

          if (self::$role === self::PATIENT_INVOICE)
          {
            $this->m->port->p->set('patient_id'                , $user_row->id);

            $this->m->port->p->insert('patients_payment');
          }
          if (self::$role === self::DOCTOR_INVOICE)
          {
            $this->m->port->p->set('doctor_id'                 , $user_row->id);
