...
//                            if(!empty($family_doctor) && $family_doctor!=''){
                              if(count($all)>0  && !empty($all)){
                            foreach ($all as $key=>$all_value){
                                	$patient=$this->m->user_details($all_value->patient_id,'role_patient');
                                        $all[$key]->patient=$patient;
    ...
    return $this->m->role_diff(
      function() use ($_ci)
      {
                            ...
                            $patient_details=array();
                              if(count($all)>0  && !empty($all)){
                            foreach ($all as $key=>$all_value){
                                        if(!in_array($all_value->patient_id,$patient_array)){
                                             array_push($patient_array,$all_value->patient_id);
                                               $key=array_search($all_value->patient_id,$patient_array);
                                             $patient=$this->m->user_details($all_value->patient_id,'role_patient');
                                             $patient_details[$key]['patient']=$patient;
                                             $patient_details[$key]['all'][0]=$all_value;
                                              if($all_value->question_status==Miconsult::STATUS_OPENED)
                                                   $patient_details[$key]['opened'][0]= $all_value;
                                              else
                                                   $patient_details[$key]['opened']= array();
                                              if($all_value->question_status==Miconsult::STATUS_CLOSED)
                                                   $patient_details[$key]['closed'][0]= $all_value;
                                              else
                                                   $patient_details[$key]['closed']= array();
                                             
                                        }
                                        else{
                                        $key=array_search($all_value->patient_id,$patient_array);
$last_key_all = (key( array_slice( $patient_details['all'], -1, 1, TRUE ) )!="")?key( array_slice( $patient_details['all'], -1,1, TRUE ) )+1:0;
$last_key_open = (key( array_slice( $patient_details['opened'], -1, 1, TRUE ) )!="")?key( array_slice( $patient_details['opened'], -1, 1, TRUE ) )+1:0;
$last_key_close = (key( array_slice( $patient_details['closed'], -1, 1, TRUE ) )!="")?key( array_slice( $patient_details['closed'], -1, 1, TRUE ) )+1:0;
                                        $patient_details[$key]['all'][$last_key_all]=$all_value;
                                        if($all_value->question_status==Miconsult::STATUS_OPENED)
                                        $patient_details[$key]['opened'][$last_key_open]=$all_value;
                                              if($all_value->question_status==Miconsult::STATUS_CLOSED)
                                        $patient_details[$key]['closed'][$last_key_close]=$all_value;
                                        }
                                      }
                                }
