...
class Module extends CI_Model {
  ...
  */
  ...
  function __construct()
  {
    ...
    if (!$this->m->port->p->table_exists('doctor_modules'))
    {
      $this->load->dbforge();
      $this->m->port->p->db_select();
    ...
    if (!$this->m->port->p->table_exists('patient_modules'))
    {
      $this->load->dbforge();
      $this->m->port->p->db_select();
    ...
    if (!$this->m->port->p->table_exists('admin_modules'))
    {
      $this->load->dbforge();
      $this->m->port->p->db_select();
  ...
  public function activate($user_id = NULL, $field = NULL, $value = 1)
  {
    ...
    if ($value)
    {
      $query = $this->m->port->p->get_where($table, array('module' => $field, $id_field => $user_id, ), 1);
      if ($query->num_rows() > 0)
      {
        $this->m->port->p->set('activate', 1);
        $this->m->port->p->where('module', $field);
        $this->m->port->p->where($id_field, $user_id);
        $this->m->port->p->limit(1);
        $this->m->port->p->update($table);
      }
      else
      {
        $this->m->port->p->set('activate', 1);
        $this->m->port->p->set('module', $field);
        $this->m->port->p->set($id_field, $user_id);
        $this->m->port->p->insert($table);
      }
    }
    else
    {
