...
class Site_config extends CI_Model {
  ...
  public function db_check()
  {
    ...
    if (!$this->m->port->p->table_exists('config'))
    {
      $this->load->dbforge();
      $this->m->port->p->db_select();
      ...
      $this->dbforge->db =& $this->m->port->p;
      $this->dbforge->create_table('config', TRUE);
    }

    $this->m->port->p->db_select();
    $query = $this->m->port->p->get('config');
  ...
  public function check_default()
  {
    ...
    ) as $config_key => $config_default_value)
    {
      if (!isset($this->_config->$config_key))
      {
        ...
        $this->m->port->p->db_select();
        $this->m->port->p->set('key', $config_key);
        $this->m->port->p->set('value', $config_default_value);

        $this->m->port->p->insert('config');
      }
    }
  }
