...
<form class="form-horizontal" role="form" method="post" action="<?php echo site_url('akte/alleconsult/'.( !empty($insert) ? 'insert' : ('update/'.$entry->id) ) ); ?>" enctype="multipart/form-data" <?php echo !empty($readonly) ? 'data-readonly="readonly"' : ''; ?> <?php echo !empty($disabled) ? 'data-disabled="disabled"' : ''; ?> >
     ...
     <label for="country" class="col-sm-6 control-label"><?php //echo $this->lang->line('epres_question_doctor');?></label>
      <div class="col-sm-9">
       ...
       <div class="radio-inline family_doc_select">
           <label>
            <input type="radio" value="1" name="cyomeddoctor"  <?php if(1==$entry->doctorcheck) echo "checked='checked'";   if(!(is_array($family_doctor) && !empty($family_doctor) && isset($family_doctor))) echo "disabled='disabled' readonly='readonly'"?> />
                <?php echo $this->lang->line('patients_iconsult_cyomed_personal'); ?>
          </label>
       </div>
     </div>
    </div>
  <?php  if(is_array($family_doctor) && !empty($family_doctor) && isset($family_doctor)):?>
  <div class="form-group family_doc_list" <?php if ($entry->doctorcheck!=1) echo 'style="display: none;"';?> > 
    <label for="country" class="col-sm-6 control-label"> <?php echo $this->lang->line('patients_iconsult_cyomed_personal_chose'); ?> </label>
     <div class="col-sm-6">
...
<?php endif; endif;?>
  <div class="form-group">
    <label for="keyword<?php echo $entry->id; ?>" class="col-sm-3 control-label text-white">
      <?php echo $this->lang->line('patients_iconsult_keyword');  ?>
    </label>
    <div class="col-sm-9">
      <input type="text" class="form-control" name="keyword" id="keyword<?php echo $entry->id; ?>" value="<?php echo !empty($entry->keyword) ? form_prep($entry->keyword) : ''; ?>" placeholder="<?php echo $this->lang->line('patients_iconsult_keyword'); ?>" required />
  ...
  <div class="form-group">
    ...
    <div class="col-sm-9">
    <textarea class="form-control" rows="5" name="message" id="message<?php echo $entry->id; ?>" placeholder="<?php echo $this->lang->line('patients_iconsult_message'); ?>" required ><?php echo !empty($entry->message) ? form_prep($entry->message) : ''; ?></textarea>
    </div>
  </div> 

  <div class="form-group">
    <label for="document_date<?php echo $entry->id; ?>" class="col-sm-3 control-label text-white">
      <?php echo $this->lang->line('patients_iconsult_document_date'); ?>
    </label>
    <div class="col-sm-3">
      <div class="input-icon datetime-pick date-only">
        <input type="text" class="form-control input-sm" name="document_date" data-format="dd.MM.yyyy" id="document_date<?php echo $entry->id; ?>" value="<?php echo date('d.m.Y', !empty($entry->document_date) ? strtotime($entry->document_date) : time()); ?>" placeholder="<?php echo $this->lang->line('patients_iconsult_document_date'); ?>" />
        <span class="add-on">
    ...
    <div class="col-sm-9 col-sm-offset-3">
      ...
      <?php if (isset($entry->replies) && is_array($entry->replies) && count($entry->replies) > 0) : ?>
        <div class="tile m-t-15 m-b-15">
          <h2 class="tile-title"><?php echo $this->lang->line('patients_iconsult_answers'); ?>
          </h2>
...
<?php if($entry->id != 0) :?>
  <?php if (!isset($entry->insert) || !$entry->insert) : ?>
    <div class="form-group">
      <label for="reply_message<?php echo $entry->id; ?>" class="col-sm-3 control-label text-white">
      <?php echo $this->lang->line('patients_iconsult_answers'); ?>
      </label>
      <div class="col-sm-9">
        <textarea class="form-control" rows="5" name="reply_message" id="reply_message<?php echo $entry->id; ?>" placeholder="<?php echo 'Nachricht'; ?>" ><?php echo !empty($entry->reply_message) ? form_prep($entry->reply_message) : ''; ?></textarea> 
      </div>
    </div>
  <?php endif; ?>
<?php endif; ?>
...
<!--         <div class="col-sm-6">
        ...
        <div class="col-sm-6 text-right">
          <?php if (!isset($entry->insert) || !$entry->insert) : ?>
            <button class="btn btn-alt btn-md" type="submit"><span class="icomoon i-reply"></span> Antwort</button>
          <?php endif; ?>
        </div>
        <?php endif; ?>
      </div>
...
<!--       <p class="help-block">
  ...
  <?php if (!isset($entry->insert) || !$entry->insert) : ?>
    <div class="form-group">
      <label for="question_status<?php echo $entry->id; ?>" class="col-sm-3 control-label text-white">
  ...
  <div class="form-group">
    <div class="col-sm-12 text-right">
      ...
      <?php if (isset($confirm_btn) && $confirm_btn) : ?>
        <button class="btn btn-alt btn-lg" type="button" data-submit-location="<?php echo site_url('akte/alleconsult/confirm/'.$entry->id); ?>" ><span class="icomoon i-signup"></span>
          <?php echo $this->lang->line('patients_iconsult_reply_to'); ?>
        </button>
      <?php endif; ?>

      <?php if (isset($archive_btn) && $archive_btn) : ?>
        <button class="btn btn-alt btn-lg" type="button" data-submit-location="<?php echo site_url('akte/alleconsult/archive/'.$entry->id); ?>" ><span class="icomoon i-drawer-3"></span> 
         <?php echo $this->lang->line('general_text_button_archieve'); ?>
        </button>
      <?php endif; ?>
     
      <?php if (isset($delete_btn) && $delete_btn) : ?>
        <button class="btn btn-alt btn-lg" type="button" data-submit-location="<?php echo site_url('akte/alleconsult/delete/'.$entry->id); ?>" ><span class="icomoon i-remove-2"></span> 
