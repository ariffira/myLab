        ...
        <?php
          if (!empty($entry->feed_type))
            switch ($entry->feed_type) {
              ...
              case 'familyhistory':
                $this->ui->feed_item->icon = 'fa fa-smile-o';
                $this->ui->feed_item->title('content', '<a href="javascript:;">'.date('d.m.Y', strtotime($entry->date_added)).'</a>'.' - Family History: '.'<a href="javascript:;">'.$entry->title.'</a>');
                break;
          ...
          if (!empty($entry->feed_type))
            if (in_array($entry->feed_type, array('condition', 'diagnosis', 'medication', 'vaccination','casehistory','familyhistory' ))) {
...
if(!empty($tot_record)&&!empty($entries)&&$tot_record>5 && count($entries)>=5)
{
  ...
  echo "<a href='".site_url("akte/feed/akte/feed/?id=purple&showmore=$show_more")."' class='show_more' id='show_more".$show_more."' ><div class='btn btn-default pull-right'>Show More</div></a><div class='clear'></div>";
}
?>
  <?php endif; ?>
