$(document).ready(function() {
  ...
  owlCalendarAll.each(owlCalendarInitCallback = function() {
  	...
  	!doctorId ? (function() {
  		owlCalendar.append($('<div>r</div>').addClass('col calendar-place-holder').html('&nbsp;'));
  		...
  		for (var dayDelta = 0, thisDay = startDate.clone(); dayDelta < 28; dayDelta++, thisDay.add('d', 1)) {
  			...
  			thisDay.isoWeekday() == 1 ? col.addClass('week-start') : $.termin;
         ...
         // console.log(termin_data);
          ...
          for (var i = 0, maskEvent; maskEvent = termin_data[i]; parseInt(maskEvent.mask) == 1 && parseInt(maskEvent.mask_event_id) != 0 ? termin_data.splice(i, 1) : i++);
          	...
          	!termin_data || !termin_data.length ? $.termin : $.each(termin_data, function() {
          		var event = this,
...
$('a.appointment-link').click(function() {
	...
	var postData = {
          ...
          var url = $.siteUrl + '/termin/portal/reservation/logout_patient/' + doctorId + '/?' + $.param(postData);
          var docpos = $('#reservation_doc_'+doctorId); 
          $.loadUrl(url,docpos);
          $('html,body').animate({scrollTop: docpos.offset().top},'slow');
            //window.location = $.siteUrl + '/termin/portal/reservation/logout/' + doctorId + '/?' + $.param(postData);
        });
      ...
      $.ajax({
      	type: "POST",
      	url: $.siteUrl + '/termin/profile/doctor/member_reservations/' + doctorId,
      	data: {},
        // dataType: 'json',
        async: true,
        cache: false,
...
$.ajax({
	type: "POST",
	url: $.siteUrl + '/termin/profile/doctor/member_termins/' + doctorId,
	data: {},
        // dataType: 'json',
        async: true,
        cache: false,
...
$('.datetime.momentjs').each(function() {
	...
	time ? $(this).append(m.format('dd. DD.MM.YYYY HH:mm')) : $.termin;
});
...
$('select[name="treatment"], .chosen-select').chosen({
...
$('#terminLogout').click(function() {
	$.ajax({
		type: "POST",
		url: $.baseUrl + '../portal/index.php/both/logout',
		data: {},
      // dataType: 'json',
      async: true,
      cache: false,
	...
	for (var week = 0; week < 52; week++) {
		if (evt.repetitive && parseInt(evt.repetitive) || week == 0) {
			...
			vc = evt.mask && parseInt(evt.mask) == 1;
                    ...
                    evt.text_patient || evt.text_notes ? (poolPushEvent.title = '<span>' + evt.text_patient + '</span>' + '<br/>' + '<span>' + evt.text_notes + '</span>') : $.termin;
                    ...
                    // Color, bgColor
                    !pr && !pb ? ((poolPushEvent.className = 'btn-default'), (poolPushEvent.borderColor = '#ccc'), (poolPushEvent.textColor = '#333333')) : $.termin;
                    pr && !pb ? ((poolPushEvent.className = 'btn-warning'), (poolPushEvent.borderColor = '#d58512')) : $.termin;
                    pr && pb ? (poolPushEvent.className = 'btn-info') : $.termin;
                    !pr && pb ? (poolPushEvent.className = 'btn-primary') : $.termin;
                    vc ? ((poolPushEvent.className = 'btn-success'), (poolPushEvent.textColor = 'white'), (poolPushEvent.title = 'SchlieÃŸzeiten')) : $.termin;
...
for (var i = 0, evt; evt = data[i++];) {
	...
	pool.push(poolPushEvent = $.extend({}, evt));
                ...
                poolPushEvent.end.isValid() ? $.termin : (poolPushEvent.end = moment(evt.start).add('m', (typeof window.docSettings !== 'undefined' ? (window.docSettings.termin_default_length || 30) : 30)));
...
function eventchange(event, delta, revertFunc){
	if (confirm("Sind Sie sicher, zu dieser Änderung?")) {
		if (event.source.name == "termins") {
			var postData, start, end, start_picker,end_picker;
       		...
       		// }
       		var start_time = moment(event.start._i);
       		var end_time = moment(event.end._i);
       		var insurance = [event.insurance_private=='1'?'1':'',event.insurance_public=='1'?'2':''];
       		start = event.start.format('YYYY-MM-DD')+' '+start_time.format('HH:mm');
       		end = event.end.format('YYYY-MM-DD')+' '+end_time.format('HH:mm');
       		...
       		var ajaxPost = function(mode) {
       			...
       			$.ajax({
       				type: "POST",
       				url: $.siteUrl + '/termin/doctor/calendar/'+mode,
       				data: postData,
       				async: true,
       				cache: false,
       				beforeSend: function() {
	...
	$("#dialogApplyChangesTo").dialog("option", "buttons", [{
		...
		click: function() {
			postData = {
				...
				start_picker: event.start.format('YYYY-MM-DD HH:mm:ss'),
				end_picker: event.end.format('YYYY-MM-DD HH:mm:ss'),
				insurance: insurance,
			};
...
$('#fullcalendar').fullCalendar({
   ...
   select: function(start, end) {
   	...
   	view && view.length > 0 && view.get(0).name == 'month' ? (event.allDay = true) : $.termin;
...
dialogTerminClick = $("#dialogTerminClick").dialog({
	...
	buttons: [{
		...
		click: function() {
			...
			$.ajax({
				type: "POST",
				url: $.siteUrl + '/termin/doctor/settings/delete/ajax/' + currentEventData.id,
				data: {},
				async: true,
				cache: false,
				beforeSend: function() {
...
dialogTerminClick.dialog("close");
	...
	click: function() {
		...
		postMode = (postData.id == 'add' ? '/termin/doctor/calendar/insert_termin' : '/termin/doctor/calendar/update_termin');

		//$saveBtn.button("disable");

		$.each(dialogTerminClick.find('input, textarea, select').not(function(index, element) {
				var $this = $(this);
		...
		try {
			...
			//dateExp = exp[0].split('.');
			postData["start_picker"] = postData["startDate"] + ' ' + postData["start_time"];
			//exp = postData["end_picker"].split(' - ');
			//dateExp = exp[0].split('.');
			postData["end_picker"] = postData["endDate"] + ' ' + postData["end_time"];
	            // postData["start_picker"] && postData["start_picker"] != postData["start"] ? (postData["start"] = postData["start_picker"]) : $.termin;
	            // postData["end_picker"] && postData["end_picker"] != postData["end"] ? (postData["end"] = postData["end_picker"]) : $.termin;
	    } catch (e) {
	    }
          ...
          var ajaxPost = function(mode) {
            ...
            if(dialogTerminClick.find('input[name="mask"]').is(':checked')){
            	var emailRegex = new RegExp(/^(("[\w-\s]+")|([\w-]+(?:\.[\w-]+)*)|("[\w-\s]+")([\w-]+(?:\.[\w-]+)*))(@((?:[\w-]+\.)*\w[\w-]{0,66})\.([a-z]{2,6}(?:\.[a-z]{2})?)$)|(@\[?((25[0-5]\.|2[0-4][0-9]\.|1[0-9]{2}\.|[0-9]{1,2}\.))((25[0-5]|2[0-4][0-9]|1[0-9]{2}|[0-9]{1,2})\.){2}(25[0-5]|2[0-4][0-9]|1[0-9]{2}|[0-9]{1,2})\]?$)/i);
      				...
      				else if(postData["email"] && !emailRegex.test(postData["email"]))
      				{
      					alert("Please enter valid email address.");
      					return false;
      				}
      				else if(postData["patient_id"]==""){
      					alert("Please enter patient id.");
      					return false;
      				}
      				else
      				{
            		if(confirm("sind Sie sicher für die Änderung!")){
            			ajaxPost('/akte/reservation/make_reservation');
            		}
            	}
            }
            else if (currentEventData.repetitive == '1') {
            	$("#dialogApplyChangesTo").dialog("option", "buttons", [{
            		text: 'Nur diese termin',
            		click: function() {
            			ajaxPost('/termin/doctor/calendar/mask_termin');
            			$(this).dialog("close");
            		},
            	}, {
            		text: 'Alle aktuellen Serientermin',
    ...
    close: function() {
    	dialogTerminClick.find('.reservation').hasClass('hidden')?'':dialogTerminClick.find('.reservation').addClass('hidden');
		dialogTerminClick.find('#isUser, #notUser').hasClass('active')?dialogTerminClick.find('#isUser, #notUser').removeClass('active'):'';
      ...
      open: function(event, ui) {
        ...
        $dialog.find('.dialog-start-date').find('span').html(currentEvent.start.format('Do MMMM YYYY'));

        var $type = $dialog.find('input[name="insurance[]"]');

        $dialog.find('#startDate').val(currentEvent.start.format('YYYY-MM-DD'));
        $dialog.find('#start_time').val(currentEvent.start.format('HH:mm:ss'));
        $dialog.find('#endDate').val(currentEvent.end.format('YYYY-MM-DD'));
        $dialog.find('#end_time').val(currentEvent.end.format('HH:mm:ss'));
...
dialogTerminClick.find('input[name="insurance[]"]').click(function() {
	var tv = $(this).val();
	$(this).prop('checked') ? (tv == '0' ? dialogTerminClick.find('input[name="insurance[]"]').not(this).prop('checked', false) : dialogTerminClick.find('input[name="insurance[]"]').filter('[value="0"]').prop('checked', false)) && dialogTerminClick.find('input[name="mask"]').prop('checked', false) : $.termin;
	dialogTerminClick.find('.reservation').addClass('hidden');
	dialogTerminClick.find('#isUser, #notUser').hasClass('active')?dialogTerminClick.find('#isUser, #notUser').removeClass('active'):'';
});

dialogTerminClick.find('input[name="mask"]').click(function() {
	dialogTerminClick.find('input[name="insurance[]"]').prop('checked', false);
	dialogTerminClick.find('.reservation').removeClass('hidden');
});
...
$('[data-toggle-type]').each(function() {
	...
	!type || !$calendarTarget ? $.termin : $(this).click(function() {
		...
		switch (type) {
			...
			relatedEvents = $calendarTarget.fullCalendar('clientEvents', function(event) {
				if (event.insurance_public == '0' && event.insurance_private == '1') {
					...
					$btn.hasClass('active') ? (arrIndex >= 0 ? event.className.splice(arrIndex, 1) : $.termin) : (arrIndex >= 0 ? $.termin : event.className.push('hidden'));
          ...
          //   $btn.hasClass('active') ? (arrIndex >= 0 ? this.className.splice(arrIndex, 1) : $.termin) : (arrIndex >= 0 ? $.termin : this.className.push('hidden'));
...
$('#inputCoordinate').click(function() {
	...
	if(strPostalCode = 'undefined')
	{
		strPostalCode = $('#zip').val();
	}
	if(strLocality = 'undefined')
	{
		strLocality = $('#city').val();
	}
	if(strStreet = 'undefined')
	{
		strStreet = $('#address').val();
	}
	
	/*console.log("strPostalCode=>"+strPostalCode);
	console.log("strLocality=>"+strLocality);
	console.log("strStreet=>"+strStreet);
	console.log("strStreetAdditional=>"+strStreetAdditional);*/
...
$('.button-regular-termin-remove').each(function() {
	$(this).click(function() {
        ...
        (terminId = $formGroup.data('regular-termin-id')) ? (
        	window.location = $.siteUrl + '/termin/doctor/settings/delete/r/' + terminId
        	) : $.termin;
    }
});
});
...
$('.input-termin-single').click(function() {
  ...
  $('[ia-action="reservation-action"]').click(function() {
  	...
  	if (action) {
  		...
  		$.ajax({
  			type: "POST",
  			url: $.siteUrl + '/termin/doctor/termin/action/' + action + '/y',
  			data: postData,
  			async: true,
  			cache: false,
  			beforeSend: function() {
...
$("#dialogBearbeiten").dialog({
	...
	buttons: {
		"Speichern": function() {
			...
			postData.id == 'add' ? console.log('exit without saving.' + dialogTerminClick.dialog("close")) : $.termin;
			...
			$.ajax({
				type: "POST",
				url: $.siteUrl + '/termin/doctor/calendar/update_termin',
				data: postData,
				async: true,
				cache: false,
				beforeSend: function() {
...
close: function() {
  ...
  open: function(event, ui) {
   ...
   refreshDialog(bData);
      ...
      $dialog.find('#startDate').val(bData.start.format('YYYY-MM-DD'));
      $dialog.find('#start_time').val(bData.start.format('HH:mm:ss'));
      $dialog.find('#endDate').val(bData.end.format('YYYY-MM-DD'));
      $dialog.find('#end_time').val(bData.end.format('HH:mm:ss'));
	...
	buttons: {
		"Speichern": function() {
			...
			else
			{
				$.ajax({
					type: "POST",
					url: $.siteUrl + '/termin/doctor/termin/new_appointment',
					data: postData,
					async: true,
					cache: false,
					beforeSend: function() {
...
// var searchKeyword = postData.medical_specialty;
// var searchRadius = postData.distance;
var markers = [];
var infoWindows = [];
var gMarkers = [];
var gInfoWindows = [];
...
function showPosition(position) {
	...
	if (postData && postData.location) {
		$('.location[data-item-prop="keywords"').html(postData.location || 'NaN');
		codeCenter(postData.location);
	} else {
...
function codeCoordinate(doctor) {
	...
	$('#search .map-popup').find(".name a").text(doctor.academic_grade + ' ' + doctor.name + ' ' + doctor.surname).attr('href', $.siteUrl + '/termin/profile/doctor/member/' + doctor.id);
	...
	$('#search .map-popup').find(".actions .btn").attr('href', typeof doctor.id != 'undefined' ? ($.siteUrl + '/termin/profile/doctor/member/' + doctor.id) : (doctor.link || '#noanchor_ihrarzt24'));
...
function searchDoctors() {
  //console.log(postData.medical_specialty);
  $('.specialty[data-item-prop="keywords"').html(postData.medical_specialty + '(e/en)');
 // console.log(postData.medical_specialty + ' in der NÃ¤he von ' + postData.location);
 placesService.radarSearch({
 	'keyword': postData.medical_specialty + ' in der NÃ¤he von ' + postData.location,
 	'location': myLatlng,
 	'radius': postData.distance,
 }, function(results, status) {
    ...
    for (var i = 0, place; place = places[i]; i++) {
        ...
        google.maps.event.addListener(marker, 'click', function() {
        	...
        	placesService.getDetails({
                ...
                $.ajax({
                	type: "POST",
                	url: $.siteUrl + '/termin/profile/doctor/google/' + place.placeReference,
                	data: {
